 ...
 * Centralized configuration for all historical time periods
 */
 ...
 */
function validatePeriodsConfig() {
    const result = {
        isValid: true,
        errors: [],
        warnings: []
    ...
    if (!Array.isArray(HISTORICAL_PERIODS)) {
        result.isValid = false;
        result.errors.push('HISTORICAL_PERIODS must be an array');
        return result;
    }

    if (HISTORICAL_PERIODS.length === 0) {
        result.isValid = false;
        result.errors.push('HISTORICAL_PERIODS cannot be empty');
        return result;
    }

    HISTORICAL_PERIODS.forEach((period, index) => {
        if (window.Validators) {
            const validation = window.Validators.validatePeriod(period);
            if (!validation.isValid) {
                result.errors.push(`Period ${index}: ${validation.errors.join(', ')}`);
                result.isValid = false;
            }
...
/**
 * Get periods configuration with validation
 * @returns {Array} Validated periods array
 */
function getHistoricalPeriods() {
    const validation = validatePeriodsConfig();
    
    if (!validation.isValid) {
        console.error('[HistoricalMap] Periods configuration validation failed:', validation.errors);
        throw new Error(`Invalid periods configuration: ${validation.errors.join(', ')}`);
    }

    if (validation.warnings.length > 0) {
        console.warn('[HistoricalMap] Periods configuration warnings:', validation.warnings);
    }

    return [...HISTORICAL_PERIODS]; // Return a copy to prevent mutations
}
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        ...
        validatePeriodsConfig
    };
}

// Make available globally
window.PeriodsConfig = {
    HISTORICAL_PERIODS,
    getHistoricalPeriods,
    validatePeriodsConfig
};